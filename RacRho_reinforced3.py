...
#Modify the parameter "K_l = 0.10" if one wants to study the influence of substrate stiffness
#Modify the parameter 'conf_width = 10.0' to simulate different confinements.
...
def migration_simulator(uniform, duro, K_s, K_a, K_l, conf_width, gama_sub, opt, nnum):
    ...
    Nsteps = int(round(6000/dt/1e4)*1e4) ;  #  time steps
    ...
    fname = 'Biphasic1'+'_W'+str(conf_width)+'_R'+str(conf_R)+'_Uni'+str(uniform)+'_Kl'+str(round(kl_id*100)/100) ;
    ...
    #   Vt_all = np.zeros([nnum, Nsteps]); #tangent speed w.r.t. Vs
    ...
    for i in range(Nsteps-1):
        ...
        #RacRho_last = usf.Assemble_RacRho(rac_an, rac_in, rho_an, rho_in, Rac_cyto[i], Rho_cyto[i], nnum)
        #rac_an = Raca_mb[:,i];   rac_in = Raci_mb[:,i];
        #rho_an = Rhoa_mb[:,i];   rho_in = Rhoi_mb[:,i];
        #Rcn = Rac_cyto[i];       Rhocn = Rho_cyto[i];
        #RacRho_current = usf.assign_values(RacRho_last) ;  # assign last step values as the current step trial values
        ...
        while disp_tol>1e-5 and iteration < 2:
            ...
            repulsive_force = T.repulsive_tangent_force(K_repulsive,  angles_between_fibers, nm_pts_vx, nm_pts_vy, thresh_angle);
            ...
            while vm_eps > 1e-6 and vm_iter < 4:
                ...
                Force_Vm=T.modified_friction_viscosity(vec_edges, K_memb, edge_L0, Force_ck_new, eta_memb, v0, Vs, Vm0, Vp, mu_coef, fr_coef, Fsub_val, Fstall, \
            ...
            #if R_nuc>3.5: pdb.set_trace()
            #V_nuc = T.curved_nucleus_velocity(nm_pts_vx, nm_pts_vy, Fst, Force_ck_new, R_nuc, eta_nuc, xnuc_pst, ynuc_pst, conf_R, conf_width);
        ...
        #    F_pro_all[:,i+1]= F_pro;  #Force_correct[0]; # protrusion forces in nucleus-membrane direction
        #    Vt_all[:,i+1]= Vm;
    ...
    #if fvr_out == 1:
    #    np.savetxt(fileVr, Vr_all.transpose(),  fmt='%.6e', delimiter='\t');
    #    np.savetxt(fileVp, Vp_all.transpose(),  fmt='%.6e', delimiter='\t');
    #    np.savetxt(fileVt, Vt_all.transpose(),  fmt='%.6e', delimiter='\t');
    #    np.savetxt(fileFpro, F_pro_all.transpose(),  fmt='%.6e', delimiter='\t');
