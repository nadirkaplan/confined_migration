...
#One can modify 'K_a = 3.0' and 'gama_sub = 0.10' to perform parametric studies on viscoelastic parameters
...
def migration_simulator(uniform, duro, K_s, K_a, K_l, conf_width, gama_sub, opt, nnum):
    ...
    Nsteps = int(round(4000/dt/1e4)*1e4) ;  #  time steps
    ...
    fname = 'Biphasic1'+'_W'+str(conf_width)+'_R'+str(conf_R)+'_Uni'+str(uniform)+'_Ka'+str(round(ka_id*100)/100)+'_eta'+str(round(gama_id*100)/100) ;
...
K_l =  0.10 ; 
gama_sub = 0.10 ;  
uniform = int(4) ;  #1 for refinement study 1; 2 refinement study 2 ; 
duro = 0 ;         #0 for constant; 1 for durotactic; 2 for anti-duro ; 
